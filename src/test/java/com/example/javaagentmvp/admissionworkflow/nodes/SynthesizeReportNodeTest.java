...
class SynthesizeReportNodeTest {
    ...
    @Test
    void buildUserPromptForRankUsesRankClosingInstruction() {
        WorkflowContext context = new WorkflowContext("run-4", "600分在湖南省的排名");
        Map<String, Object> finalResult = Map.of(
                "intent", "RANK",
                "summary", "已查询到 4 条位次记录",
                "rankResult", Map.of(
                        "count", 2,
                        "ranks", List.of(Map.of(
                                ...
                                "rank_max", 2369,
                                "segment_count", 96,
                                "source_url", "https://example.com/hunan-2025"))));

        String prompt = SynthesizeReportNode.buildUserPrompt(context, finalResult);

        assertThat(prompt).contains("官方一分一段表");
        assertThat(prompt).contains("2274-2369");
        assertThat(prompt).contains("https://example.com/hunan-2025");
        assertThat(prompt).contains("请根据以上位次数据撰写完整回答");
        assertThat(prompt).doesNotContain("志愿分析报告");
    }
    ...
    void synthesizesRankReportDeterministicallyWithoutLlm() {
        ...
        finalResult.put("rankResult", Map.of(
                "count", 1,
                "ranks", List.of(Map.of(
                        "year", 2025,
                        "province", "安徽",
                        ...
                        "rank_max", 3415,
                        "segment_count", 130,
                        "source_url", "https://example.com/anhui-2025"))));
        context.put(FormatResponseNode.KEY_FINAL_RESULT, finalResult);
        ...
        @SuppressWarnings("unchecked")
        Map<String, Object> updated = context.get(FormatResponseNode.KEY_FINAL_RESULT, Map.class);
        String report = String.valueOf(updated.get("report"));
        assertThat(report).contains("rank-result-table");
        assertThat(report).contains("3,286–3,415");
        assertThat(report).contains("130人");
    }
}
