...
run $SUDO dnf install -y postgresql18-server postgresql18
run $SUDO /usr/pgsql-18/bin/postgresql-18-setup initdb || true
run $SUDO systemctl enable postgresql-18
run $SUDO systemctl start postgresql-18
echo "PostgreSQL 18 installed (RHEL 9 family)" >&2
